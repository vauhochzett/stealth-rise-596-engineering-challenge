...
from api.openai import determine_commodity_group, extract_procurement_request
...
async def add_request(request: ProcurementRequest):
    # Auto-classify the commodity group from the rest of the request data
    existing_groups = {req.commodity_group for req in api.data.get_requests()}
    request = request.model_copy(
        update={
            "commodity_group": await determine_commodity_group(
                request, existing_groups
            )
        }
    )
